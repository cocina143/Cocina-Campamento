      ...
      <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between pb-4 border-b border-stone-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-stone-900">Personas con Alergias</h2>
          </div>
        ...
        <div className="mt-4 bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2 text-xs text-amber-800">
          ...
          <span>Añade las alergias de cada persona (ej: "melón", "tomate"). La app te alertará si un plato lo contiene.</span>
        </div>
              ...
              <div key={persona.id} className="bg-stone-50 rounded-2xl p-4 border border-stone-200">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => handleDeletePersona(persona.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg">
                    ...
                    persona.alergias.map((alergia, idx) => (
                      <span key={idx} className="inline-flex items-center gap-1 bg-red-100 text-red-800 text-xs font-semibold px-2.5 py-1 rounded-full">
                        ...
                        {editingId === persona.id && (
                          <button onClick={() => handleRemoveAlergia(persona.id, idx)} className="hover:text-red-900">
                            <X className="w-3 h-3" />
                          </button>
                        )}
                      </span>
                  ...
                  <div className="mt-3 pt-3 border-t border-stone-200 space-y-2">
                    <div className="flex gap-2">
                      <input
                        ...
                        placeholder="Nueva alergia (ej: melón)"
                        className="flex-1 border border-stone-300 rounded-lg p-2 text-xs"
                      />
                      <button onClick={() => handleAddAlergia(persona.id)} className="bg-orange-100 text-orange-700 px-3 py-2 rounded-lg text-xs font-bold hover:bg-orange-200">
                        + Alergia
                      </button>
                    </div>
                    <textarea
        ...
        <button onClick={handleSave} className="w-full mt-4 bg-stone-900 text-white font-bold py-3 rounded-2xl hover:bg-stone-800 transition-all">
